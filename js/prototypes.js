...
Insurance.prototype.quoteInsure = function() {
  const base = 2000;
  let cant = 0;
  ...
  forms.addEventListener("submit", e => {
    ...
    if (brand === "")
      userInterface.showError(
        "Verifique que no existan campos vacios",
        "error",
      );
    else {
...
UserInterface.prototype.showError = function(message, type) {
  const div = document.createElement("div");
  if (type === "error") div.classList.add("mensaje", "error");
...
UserInterface.prototype.showResult = function(insurance, cant) {
